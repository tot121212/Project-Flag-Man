...
# stores previous frames is_on_floor state
...
func _physics_process(delta):
	...
	if not is_on_floor(): # gravity
		...
		if Input.is_action_pressed("down"):
			velocity.y = move_toward(velocity.y, gravity * 2, gravity * delta * 2) # quickly go down if pressing down key
	...
	if direction.x != 0 && direction.x == prev_direction.x: # move if direction and prev is equal to cur direction.x
		x_acceleration *= 1.1
	...
	prev_direction = direction # save prev direction
	move_and_slide() # then enact physics
