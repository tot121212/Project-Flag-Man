...
func actor_setup():
	...
	synced = true
	print("NavigationComponent: Synced")
	...
	if synced:
		...
		if closest_point:
			if _check_path(origin, closest_point) != null:
				...
				print("NavigationComponent: Path is null")
...
func _calculate_alternate_path(origin : Vector2, destination : Vector2, nav_layers : int = 1, optimize : bool = true, rid : RID = default_rid):
	var offset = Vector2(32, 0)
	var alt_1 = NavigationServer2D.map_get_path(default_rid, origin + offset, destination, optimize)
	var alt_2 = NavigationServer2D.map_get_path(default_rid, origin - offset, destination, optimize)
